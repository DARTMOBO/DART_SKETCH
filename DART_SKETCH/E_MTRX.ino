...
// BLINK-FIX: in LIME/Matrix_Pads, bank1 was shared between:
// - blinker bits (data_LT 1..32 -> indices 0..31 (+page))
// - matrix LED status bits (mu 0..15 -> indices 0..15 (+page))
// This caused velopads/buttons to accidentally set "blinker" bits via matrix writes.
// We move the matrix LED-status window to a safe area inside the same bank:
//   bank1 indices 32..47 (+page) are reserved for MAX7219 status (mu 0..15).
#define MATRIX_BIT_BASE 32
...
void single_h(byte number_of_unit, byte sprite, byte invert, byte send_) {
  ...
  if (sprite > 46) {
    ...
    // remap della numerazione di "led" (cioè data_LT-1)
    ...
    lc.setRow(number_of_unit, 6, data_VA[sprite_pos + page] ^ m, send_);
    lc.setRow(number_of_unit, 1, data_MA[sprite_pos]          ^ m, send_);
    lc.setRow(number_of_unit, 5, data_MI[sprite_pos]          ^ m, send_);
    lc.setRow(number_of_unit, 3, data_MODE[sprite_pos]         ^ m, send_);

    lc.setRow(number_of_unit, 7, data_DM[sprite_pos]          ^ m, send_);
    lc.setRow(number_of_unit, 2, data_QW[sprite_pos]       ^ m, send_);
    lc.setRow(number_of_unit, 4, data_TY[sprite_pos + page]  ^ m, send_);
    lc.setRow(number_of_unit, 0, data_LT[sprite_pos]         ^ m, send_);
  }
...
void matrix_restore (boolean numero)
{
 ...
 /* for(int unit = 0; unit < 8; unit++)  // aggiorno l'array STATUS che contiene la situazione di tutte le matrici
      ...
      single_h(matrix_remap[remapper(unit)],                             // quale matrice                                                     
               data_LT[remapper(unit)+16],                           // quale sprite 
  ...
  for (int unit = 0; unit < 12; unit++)
  {
    ...
    if (mu < 8)
    {
      single_h(mu,                  // quale matrice
               data_LT[r],        // quale sprite
               bit_read(1, MATRIX_BIT_BASE + mu + off),
               0);                  // send = 0
    }
  }
...
void led_enc_exe_matrix() {
  ...
  // effetti LED attivi?
  if (data_QW[general_mempos] == 0) return;
...
void ledControl_matrix (byte chann, byte stat)  {
    ...
    if (data_LT[chann] == 0) return;  // GUARD: nessun LED assegnato → nessuna azione
      bit_write(1, MATRIX_BIT_BASE + matrix_remap[chann] + page, stat);  
