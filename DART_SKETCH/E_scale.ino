...
void scale_learn(byte pitch) {
  if (data_LB[touch_mempos[0]] == 0 || data_LB[touch_mempos[1]] == 0) { // se touch premuto/toccato
    if (data_DM[spinner_mempos[0]] > 1) {
...
void update_scala(byte quale_spinner) { // quale_spinner è 0 top 1 side
  ...
  // i primi 7 BIT sono dentro data_VA
  ...
  for (byte i = 0; i < 7; i++) {
    bitWrite(scala[quale_spinner + (page / max_modifiers) * 2], i, bitRead(data_VA[spinner_mempos[quale_spinner] + page], i));
    // bitWrite(scala[0], i, bitRead(data_VA[spinner_mempos[quale_spinner] + page], i));
  }
  
  for (byte i = 0; i < 5; i++) {
    bitWrite(scala[quale_spinner + (page / max_modifiers) * 2], i + 7, bitRead(EEPROM.read(spinner_mempos[quale_spinner] + 320 + (page / max_modifiers * 512)), i));
    // bitWrite(scala[0], i + 7, bitRead(EEPROM.read(spinner_mempos[quale_spinner] + 320 + (page / max_modifiers * 512)), i));
  }
