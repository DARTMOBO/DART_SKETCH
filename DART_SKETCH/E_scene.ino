...
static void convoy_send_subject(byte mem_chan, byte value)
{
  ...
  byte t = data_TY[mem_chan];
  if (t >= 144) {
    byte group = (t - 144) / 16;
    if (group == 3) {
      midiSendFiltered(t - 16, data_VA[mem_chan], value, 0);
    }
    else if (group == 2) {
      midiSendFiltered(t, data_VA[mem_chan], value, 0);
    }
  }
  #if (DMX_active == 1)
  if (eeprom_preset_active == 1 && data_DM[mem_chan] > 0) {
    DmxSimple.write(data_DM[mem_chan], value * 2);
  }
  #endif
}
...
// - encoder generico (mode 19): data_LT[chan] = v*2
// Nota: SOLO se data_DM[chan] == 2 (pot emulation)
// ============================================================
static inline void convoy_sync_encoder_base(byte chan, byte v)
{
  if (data_DM[chan] != 2) return;          // non è pot-emulation → niente sync

  byte mode = data_MODE[chan];

  // spinner top/side (mode 21/22): base = encodervaluepot[0/1] (0..1023)
  if (mode == 21 || mode == 22)
  {
    if (chan == spinner_mempos[0]) { encodervaluepot[0] = ((int)v) << 3; return; }  // v*8
    if (chan == spinner_mempos[1]) { encodervaluepot[1] = ((int)v) << 3; return; }  // v*8
    return;
  }

  // encoder generico (mode 19): base = data_LT[chan] (0..255), out = data_LT/2
  if (mode == 19)
  {
    data_LT[chan] = (byte)(v << 1);       // v*2 (0..254)
  }
}
...
// NOTA: NON usare data_MA[]: è lo stato quadratura (0..3) in updateEncoder()!
// ============================================================


#define MORPH_TEST_FORCE_SCENE12  0   // 0 = modalità normale (snapshot + data_VA)
...
// posizione per i 2 encoder fisici (indicizzati via spinner_mempos[0/1])
...
byte scene_record_blink_led = 0;        // 1..N (come data_LT), 0=nessuno
...
void scene_build_subject_list()
{
  ...
  for (byte c = 0; c < max_modifiers; c++) {
    ...
    // sicurezza: data_MODE_readmode ha 40 elementi (0..39)
    ...
    //    (nel tuo sketch: data_DM[chan] == 2)
    //    e con mode encoder/spinner (19 / 21 / 22).
    byte isAnalogSubject = (data_MODE_readmode[mode] == 1);
    byte isEncSubject = 0;
    if (!isAnalogSubject) {
      if ((mode == 19 || mode == 21 || mode == 22) && (data_DM[c] == 2)) {
        isEncSubject = 1;
      }
    }
    if (!isAnalogSubject && !isEncSubject) continue;

    // TAG via data_TY: gruppo PC = 3
    // data_TY è uno status MIDI “pronto”: 144 + (group*16) + channel
    byte t = data_TY[c];
...
void scene_control_pot()
{
  ...
  int pot_confronto = abs((data_LB[chan] * 4) - valore);
  ...
  if (data_MI[chan] > data_MA[chan]) {
    level = (byte)(127 - level);  // 0<->127, 1<->126 ... (scala completa invertita)
  }

// quale scena controlla questo pot (da data_VA)
  byte scene_index = data_VA[chan] & 0x07;   // 0..7
  ...
  for (byte i = 0; i < scene_subject_count && i < SCENE_PARAM_COUNT; i++) {
    ...
    for (byte s = 0; s < SCENE_COUNT; s++) {
      ...
      if (contrib > final_value) final_value = contrib;
	    ...
	    // ============================================================
	    if (data_MODE[c] >= 11 && data_MODE[c] <= 15)
	    {
	      // data_LB[] è 0..255; nello sketch lo converti spesso a 0..127 con >>1.
	      // Quindi qui usiamo una conversione minimale e coerente: v(0..127) -> data_LB(v*2).
	      data_LB[c] = (byte)(final_value << 1);
...
void scene_record_execute(byte scene_index)
{
  ...
  for (byte i = 0; i < scene_subject_count && i < SCENE_PARAM_COUNT; i++) {
    ...
    // - POT analogici: data_LB[] (0..255) -> >>1
    // - ENCODER (mode 19) in pot-emulation (data_DM==2): data_LT[] (0..255) -> >>1
    // - SPINNER (mode 21/22) in pot-emulation (data_DM==2): encodervaluepot[idx] (0..1023) -> /8
    // ------------------------------------------------------------
    byte v = 0;
    byte mode = data_MODE[c];

    // Caso A: pot/analogico (readmode==1)
    if (mode < 40 && data_MODE_readmode[mode] == 1) {
      v = (byte)(data_LB[c] >> 1);
    }
    // Caso B: encoder generic (mode 19) in pot-emulation
    else if (mode == 19 && data_DM[c] == 2) {
      v = (byte)(data_LT[c] >> 1);
    }
    // Caso C: spinner (mode 21/22) in pot-emulation
    else if ((mode == 21 || mode == 22) && data_DM[c] == 2) {
      byte idx = SCENE_CHAN_INVALID;
      if (c == spinner_mempos[0]) idx = 0;
      else if (c == spinner_mempos[1]) idx = 1;
...
void scene_record_button()
{
  ...
  // (qui non ci serve la velocity)
  push_buttons_lettura_stage(0);

  // Pressione confermata (debounced)
  if (pb_pressed()) {
    // Pressione confermata: arma il record.
    scene_record_armed = 1;

    // memorizziamo quale LED è associato a questo pulsante RECORD
    // CTRL-F: SCENE_REC_LED_FROM_data_LT
    scene_record_blink_led = data_LT[chan];   // LED scelto dal preset per il pulsante SCENE_RECORD
...
void scene_eeprom_save_all()
{
  ...
  // ============================================================
  // CTRL-F: EEPROM_EMU_COMMIT_ON_SCENE_SAVE
  // Salvataggio scene: su SAMD commit su flash solo a fine scrittura (preserva usura).
  // ============================================================
...
// - riconoscimento page-aware a monte: data_TY[enc_chan + page] == 244
// - usa data_VA[enc_chan] come target scene (NO +page: scene/morph esclude pageswitch)
// - usa data_MA[enc_chan] come posizione morph 0..127 (riuso RAM)
// - quando cambia owner (o “riparte” un altro encoder), scatta la FOTO base
    // scene_morph_encsc(numero);
void scene_morph_encsc(byte enc_chan)
{
  // determina quale dei 2 encoder fisici è (0 o 1) usando spinner_mempos[]
  byte idx = 0;
  if (enc_chan == spinner_mempos[1]) idx = 1;

  // 1) target scena deciso da data_VA (NO pageswitch)
  byte target_scene = (data_VA[enc_chan] & 0x07);
  ...
  {
    byte ownerIdx = 0;
    if (morph_owner == spinner_mempos[1]) ownerIdx = 1;
...
convoy_snapshot(morph_base);
  ...
  // Usiamo lo stesso "signed speed" dello spinner (data_MI[]: 32=1x, >32=più veloce, <32=inversione)
  // Così puoi:
  ...
  // ------------------------------------------------------------
  const int8_t  s     = (int8_t)data_MI[enc_chan] - 32;               // -32..+32
  ...
  int8_t baseDir = 0;
  if (data_LB[enc_chan] == 65) baseDir = +1;
  else if (data_LB[enc_chan] == 63) baseDir = -1;
