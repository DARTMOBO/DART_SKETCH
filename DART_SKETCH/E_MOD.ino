...
void beam() { //----------------------------------------------------- BEAM SENSOR
  ...
  } else if (qwertyvalue[chan] == 1) { ///////////// impostazione note trigger
...
void seq() {
  ...
  if (valore < lower_val && lastbutton[chan] == 1) { ///// button pushed
    ...
    if (maxvalue[chan] > 0) {
      switch (qwertyvalue[chan]) { // qwertyvalue[chan] usato come contatore > 159 sono cc
        case 0:
      ...
      
      {
        if (padDecay[padNum] > 0) { 
          ...
          if (padDecay[padNum] == 1) {
            midiOut(typetable[PADS_mempos + (page)], valuetable[PADS_mempos + (page)] + (padNum * 2), 0);
          }
        }
      } 
    }
