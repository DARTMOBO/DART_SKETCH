...
void qwerty_out(byte onoff, byte chan_, byte add) {
  switch (onoff) {
    case 1: {
      ...
      #if (hid_keys == 1)  
      if (chan_ < 25 && chan_ > 0) {
        Keyboard.press(pgm_read_byte(qwertymod + chan_ + add));
      } else if (chan_ > 31) {
        Keyboard.press(chan_ + add); // normale tabella ascii //
      }
      #if (hid_mouse == 1)
      else if (chan_ != 31) {
        Mouse.press(chan_ - 24 + add); // 25 26 27 28 29 30 -
      }
      #endif
      #endif
      #endif
    ...
    case 0: {
      ...
      #if (hid_keys == 1)    
      if (chan_ < 25 && chan_ > 0) {
        Keyboard.release(pgm_read_byte(qwertymod + chan_ + add));
      } else if (chan_ > 31) {
        Keyboard.release(chan_ + add);
      }
      #if (hid_mouse == 1)
      else if (chan_ != 31) {
        Mouse.release(chan_ - 24 + add);
      }
      #endif
      #endif
      #endif  
