...
// CTRL-F: DMX_CPU_LOCK_32U4
#if !defined(__AVR_ATmega32U4__)
  #error "DART DMX: questa build è pensata solo per ATmega32U4 (Leonardo)."
#endif
...
// volatile uint16_t lettura_int;
static uint8_t dmxMax = 16;     // 0..127 /* Default to sending the first 16 channels */
static uint8_t dmxStarted = 0;
static uint8_t dmxState = 0;   // 0..127
...
// CTRL-F: DMX_INTERNAL_STATIC
static void dmxBegin();
static void dmxEnd();
static void dmxSendByte(volatile uint8_t);
static void dmxWrite(int, uint8_t);
static void dmxMaxChannel(int);
...
#define BITS_PER_TIMER_TICK (F_CPU / 31372)
 // #define BITS_PER_TIMER_TICK (F_CPU / 62744)
...
static void dmxSendByte(volatile uint8_t value)
{
...
ISR(ISR_NAME,ISR_NOBLOCK) {
 ...
 // m 
 TIMER2_INTERRUPT_DISABLE();
 //  lettura_int = (cs_4_3.capacitiveSensorRaw(1)/8);
...
static void dmxWrite(int channel, uint8_t value) {
...
static void dmxMaxChannel(int channel) {
  if (channel <=0) {
