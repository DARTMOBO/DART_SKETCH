  ...
  for (channel = 0; channel < 20; channel++) { 
    ...
    if (lastbutton[encoder_mempos[0]] == 64 || dmxtable[general_mempos] == 0) { // 64 = no encoder action - the MAIN spinner has priority over any other action.
      ...
      if (channel == 1) {
        ...
        if (lastbutton[touch_mempos[0]] == 1) {
      ...
      //  digitalWrite(14, HIGH);       // cambio di canale lettura - diodi
        ...
        const byte chan_enc = 14;   // in STRATOS: channel == memoryposition
          ...
          if ((uint32_t)(now - enc_lock_last_us[chan_enc]) < (uint32_t)ENC_LOCKOUT_US) {
            ...
            // Però aggiorniamo la fase quadrature (maxvalue) per non creare salti.
            ...
            MSB[1] = (byte)(s_lock >> 1);
            LSB[1] = (byte)(s_lock & 1);
            maxvalue[chan_enc] = s_lock;
            lastbutton[chan_enc] = 64;
          } else
        #endif
                  {
            ...
            #if (ENABLE_ENC_LOCKOUT == 1)
              if (lastbutton[chan_enc] != 64) enc_lock_last_us[chan_enc] = now;  // usa lo stesso timestamp
            #endif
          }
      }
